...
export async function GET(req: NextRequest) {
  ...
  if (!rawPkce) {
    return NextResponse.redirect(
      new URL("/login?authv2_error=" + encodeURIComponent("Session expired. Please try again."), req.url)
    );
  }

  let pkce: { verifier: string; state: string; school: string; orgid: string; redirectUri: string };
  try {
    pkce = JSON.parse(rawPkce);
  } catch {
    return NextResponse.redirect(
      new URL("/login?authv2_error=" + encodeURIComponent("Invalid session data. Please try again."), req.url)
    );
  }

  if (!state || pkce.state !== state) {
    return NextResponse.redirect(
      new URL("/login?authv2_error=" + encodeURIComponent("State mismatch. Please try again."), req.url)
    );
  }
  ...
  if (!tokenResult) {
    return NextResponse.redirect(
      new URL("/login?authv2_error=" + encodeURIComponent("Token exchange failed. Please try again."), req.url)
    );
  }
  ...
  const expiresIn    = typeof tokenResult.expires === "number" ? tokenResult.expires : 900;

  if (!ssToken) {
    return NextResponse.redirect(
      new URL("/login?authv2_error=" + encodeURIComponent("No access token returned. Please try again."), req.url)
    );
  }
  ...
  try {
    ...
    console.error("[auth/v2/callback] eva-apps session error:", (err as Error).message);
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      const base: StatsData = {
        total_logins: 0, total_successful_logins: 0,
        total_api_calls: 0, unique_logins: 0, failed_logins: 0, users: [],
      };
    ...
    console.error("[auth/v2/callback] Firestore error:", (err as Error).message);
  ...
  res.cookies.set("ssp_v2_pkce", "", { httpOnly: true, sameSite: "lax", path: "/", maxAge: 0 });
  ...
  if (refreshToken) {
    res.cookies.set("ssp_ss_refresh_token", refreshToken, { ...sessionCookieOpts, maxAge: 60 * 60 * 24 * 30 });
  }
  res.cookies.set("ssp_auth_type", "authv2", { ...sessionCookieOpts, httpOnly: false, maxAge: 60 * 60 * 24 * 30 });
  res.cookies.set("ssp_ss_orgid",  orgid,    { ...sessionCookieOpts, httpOnly: false, maxAge: 60 * 60 * 24 * 30 });
